...
export default function ProfileCard(profileCardData: UserProfile) {
	...
	return (
		<main className="w-full h-full pt-8 flex-row bg-gradient-to-br from-cyan-400 via-violet-400 to-pink-500">
			<h3 className='text-center mx-auto font-semibold text-xl text-white'>{profileCardData.username}</h3>
			<section
				...
				<section className='flex-row justify-evenly pt-24 pb-4 text-center'>
					<h1 className='text-center mx-auto font-bold text-2xl'>{profileCardData.email}</h1>
					<p className='text-sm'>{profileCardData?.nationality}</p>
					...
					<section className='flex justify-evenly pt-8 items-center'>
						<div className='active-container'>
							...
							{/* <span className='font-bold text-lg ml-2 text-pink-500'>{profileCardData.Favorite_Recipes.length}</span> */}
						</div>
